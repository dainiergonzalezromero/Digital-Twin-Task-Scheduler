...
Resultado heuristicaGreedyConReintentos(
    ...
    for (int intento = 0; intento < max_intentos; intento++) {
        // Si ya tenemos una solución factible, no continuamos con más reintentos
        if (mejor_resultado.factible) {
            if (debug) {
                cout << "\n\033[94m[SALIDA TEMPRANA] Solución factible ya encontrada. Terminando reintentos.\033[0m" << endl;
            }
            break;
        }
