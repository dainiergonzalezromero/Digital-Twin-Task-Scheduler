    ...
    for i in range(Number_Processors):
        for j in range(Number_Processors):
            ...
            # 4) Mist ↔ Fog 
                ...
                (i < edge_start and fog_start <= j < cloud_id) or 
                (j < edge_start and fog_start <= i < cloud_id)
            ):
                delay = 4
            ...
            # 7) Edge ↔ Cloud 
            elif (edge_start <= i < fog_start and j == cloud_id) or \
...
def Generar_Dat(Number_Tasks, Number_Processors, Processors, Tasks, Relationship, Communication ):
    ...
    output_file = os.path.join(folder, f"datos.dat")
...
def Generar_Dat_Formato2(Number_Tasks, Number_Processors, Processors, Tasks, Relationship, Communication):
    ...
    output_file = os.path.join(folder, f"datos2.dat")
    ...
    with open(output_file, "w") as f:
        ...
        f.write(f"param beta  := 0;\n")
        f.write(f"param gamma := 1;\n")
